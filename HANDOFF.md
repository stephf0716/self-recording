...
```text
Install and then run the self-recording skill (v0.3.1). Do not improvise a different memory system.
...
7. Install only this client's automation. Claude Code: scripts/install-hooks.sh. Hermes-only: skip Claude hooks; use the macOS scheduled sweep (scripts/install-autocommit-agent.sh) or a Hermes script-only cron backed by a self-contained machine-local script under $HERMES_HOME/scripts. Do not point Hermes cron directly at cloud-synced _tools; its lifecycle guard refuses to scan those paths. Other clients: AGENTS.md plus the sweep if they cannot commit themselves.
