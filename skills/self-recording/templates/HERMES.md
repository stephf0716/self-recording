<!--
TEMPLATE. Copy to the vault root as `.hermes.md`. A POINTER, NEVER A COPY.
Duplicating the contract here guarantees the two drift apart. Keep this short.
Hermes already injects AGENTS.md; this file exists so a Hermes-only session
still hits the same rules if AGENTS.md is skipped or truncated.
-->
...
top. Don't ask permission and don't stage it for review; history auto-commits (Claude
`Stop` hook, scheduled sweep, and/or Hermes cron) and any entry is revertible.
